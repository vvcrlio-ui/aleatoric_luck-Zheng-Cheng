...
private.data.dir <- file.path(getwd(), 'data', 'private')
...
#  Original seed used in the benchmark: 08544
  set.seed(x)
  d <- background %>%
  ...
  get.benchmark.predictions <- function(outcome, model = "full", data = d) {
    if(model == "full") {
      ...
      if (!is.null(ols)) {
        # BUGFIX: the original code fitted OLS and extracted beta, but never
        # wrote predictions into ols.yhat, so downstream OLS R2 became NA.
        # ols.yhat[!missing_all_predictors] <- predict(
        #   ols,
        #   newdata = imputed[!missing_all_predictors,]
        # )
        ols.yhat[!missing_all_predictors] <- predict(
          ols,
          newdata = imputed[!missing_all_predictors,]
      ...
      # Fit a logistic regression model for the three binary FFC outcomes.
      binary_outcomes <- c("eviction", "layoff", "jobTraining")
      # BUGFIX: the original condition checked the imputed outcome values:
      # if (length(unique(na.omit(imputed[, outcome]))) == 2) {
      # Amelia can turn binary outcomes into non-0/1 continuous values, which
      # skips the logit branch. Use the known binary outcome names instead.
      if (outcome %in% binary_outcomes) {
        logit_data <- imputed
        # Use the observed training response, not the imputed target outcome,
        # while keeping Amelia-imputed predictors for prediction.
        logit_data[[outcome]] <- data[[outcome]]
        logit <- tryCatch(
          glm(formula = thisFormula, family = binomial(link = "logit"), data = logit_data[!is.na(logit_data[,outcome]), ]),
          warning = function(w) {
        ...
        if (!is.null(logit)) {
          ...
          print(logit_summary)
          # BUGFIX: prediction should not depend on whether the lagged-outcome
          # coefficient name is present. The original line below only ran
          # inside the beta branch, so logit R2 became NA whenever beta was NA.
          # logit.yhat[!missing_all_predictors] <- predict(logit, newdata = imputed[!missing_all_predictors,], type = "response")
          logit.yhat[!missing_all_predictors] <- predict(
            logit,
            newdata = imputed[!missing_all_predictors,],
            type = "response"
          )
  ...
  # BUGFIX: without id.vars, melt() treats both outcome and logit_beta as ID
  # variables. That drops the logit row after account names are cleaned.
  # benchmark_beta_melted <- melt(benchmark_beta, variable.name="account", value.name="beta")
  benchmark_beta_melted <- melt(benchmark_beta, id.vars="outcome", variable.name="account", value.name="beta")
...
output_file <- file.path(seed.dir, sprintf("seed_analysis_%s_logit_fix.csv", max_seeds))

# Use base R here because readr::write_csv() can fail to open files on some
# macOS setups when the output path is synced/locked or disk space is tight.
